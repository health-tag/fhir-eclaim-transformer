...
def save_bundle_to_file(bundle: Bundle, output_folder: PathLike, file_name: str):
    with open(Path.joinpath(output_folder,file_name), "w") as f:
...
def process_all(_1ins_path: PathLike, _2pat_path: PathLike, _3opd_path: PathLike,
    ...
    #    patients_dict = manager.dict()
    #    with Pool() as pool:
    #        process_patient = pool.map_async(partial(func, b=second_arg), a_args)
    ...
    for sequence, matched in joined_opd_files.items():
        ...
        citizenId = patient.identifier[0].value
        # Coverage
        ...
        if matched.row_3opd is not None:
            ...
            account.identifier = [Identifier(type=CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-identifier-type"),code=Code("localVn"))]),system=Uri(f"https://terms.sil-th.org/hcode/5/{matched.row_1ins.main_hospital_code}/VN"), value=String(matched.row_3opd.sequence))]
            ...
            encounter.identifier = [Identifier(type=CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-identifier-type"),code=Code("localVn"))]),system=Uri(f"https://terms.sil-th.org/hcode/5/{matched.row_1ins.main_hospital_code}/VN"), value=String(matched.row_3opd.sequence))]
            ...
            encounter.location = [EncounterLocation(location=Reference(identifier=Identifier(system=Uri(f"https://terms.sil-th.org/hcode/5/{matched.row_1ins.main_hospital_code}/DepCode"), value=String(matched.row_3opd.clinic))))]
            ...
            for matched_row_16dru in matched.row_5odx:
                ...
                condition.code = CodeableConcept(coding=[Coding(system=Uri("http://hl7.org/fhir/sid/icd-10"), code=Code(matched_row_16dru.diagnosis_icd10))])
            ...
            for matched_row_16dru in matched.row_6oop:
                ...
                procedure.code = CodeableConcept(coding=[Coding(system=Uri("http://hl7.org/fhir/sid/icd-9-cm"), code=Code(matched_row_16dru.operation), display=String("Patient referral"))])
                ...
                procedure.performer = [ProcedurePerformer(actor=Reference(type=Uri("Practitioner"), identifier=Identifier(system=Uri(f"https://terms.sil-th.org/id/th-doctor-id"), value=String(matched_row_16dru.dropid))))]
                procedure.id = Id(f"cid-{citizenId}-vn-{matched_row_16dru.sequence}-proc")
                procedures.append(procedure)
            # Claim (11,12)
            if matched.row_11cht is not None:
                claim = Claim.construct(status=Code("active"), use=Code("claim"), created = String(matched.row_11cht.date))
                claim.extension = [
                ...
                #]
                claim.type = CodeableConcept(coding=[Coding(system=Uri("http://terminology.hl7.org/CodeSystem/claim-type"), code=Code("institutional"))])
                ...
                if(len(matched.row_12cha) > 1):
                    ...
                    for idx,matched_row_12cha in enumerate(matched.row_12cha):
                        ...
                        claimItem.productOrService = CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-eclaim-charge-item"), code=Code(matched_row_12cha.chrgitem), display=String("ค่าบริการทางการพยาบาล (เบิกได้)"))])
            ...
            for matched_row_16dru in matched.row_16dru:
                # Claim (16)
                drug_claim = Claim.construct(created=String(matched_row_16dru.service_date), use = Code("claim"), status = Code("active"))
                ...
                if coverage is not None:
                    drug_claim.insurance = [ClaimInsurance(sequence=1, focal=Boolean(True), coverage=Reference(reference=coverage.relative_path()), preAuthRef=[String(matched_row_16dru.pa_no)])]
                drug_claim.total = Money(value=Decimal(matched_row_16dru.total), currency=Code("THB"))
                drug_claim.item = [ClaimItem(sequence=1, productOrService=CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-local-drug-code"), code=Code(matched_row_16dru.drug_id)), Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-24drug"), code=Code(matched_row_16dru.drug_id24))], text=String(matched_row_16dru.drug_name)), quantity=Quantity(value=Decimal(matched_row_16dru.amount), unit=String(matched_row_16dru.unit)), servicedDate=String(matched_row_16dru.service_date), encounter=[Reference(reference=encounter.relative_path())], unitPrice=Money(value=Decimal(matched_row_16dru.drug_price), currency=Code("THB")), net=Money(value=Decimal(matched_row_16dru.total), currency=Code("THB")))]
                drug_claim.id = Id(f"cid-{citizenId}-vn-{matched_row_16dru.sequence}-24-{matched_row_16dru.drug_id24}")
                claims.append(drug_claim)

                # MedicationDispense
                medication_dispense = MedicationDispense.construct(status = Code("completed"), medicationCodeableConcept = CodeableConcept(text=String(matched_row_16dru.drug_name), coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-local-drug-code"), code=Code(matched_row_16dru.drug_id)), Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-th-24drug"), code=Code(matched_row_16dru.drug_id24))]))
                ...
                medication_dispense.quantity = Quantity(value=Decimal(matched_row_16dru.amount), unit=String(matched_row_16dru.unit))
                medication_dispense.whenHandedOver = String(matched_row_16dru.service_date)
                medication_dispense.id = Id(f"cid-{citizenId}-vn-{matched_row_16dru.sequence}-24-{matched_row_16dru.drug_id24}")
                ...
                medication_request.extension = [Extension(url=Uri("https://fhir-ig.sil-th.org/mophpc/StructureDefinition/ex-medicationrequest-med-approved-no"), valueString=String(matched_row_16dru.pa_no))]
                if(isinstance(matched_row_16dru.drug_remark, str) and not matched_row_16dru.drug_remark):
                    medication_request.extension.append(Extension(url=Uri("https://fhir-ig.sil-th.org/mophpc/StructureDefinition/ex-medicationrequest-ned-criteria"), valueCodeableConcept=CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-eclaim-medication-ned-criteria"), code=Code(matched_row_16dru.drug_remark), display=String("เกิดอาการไม่พึงประสงค์จากยาหรือแพ้ยาที่สามารถใช้ได้ในบัญชียาหลักแห่งชาติ") )])))
                medication_request.category = [CodeableConcept(coding=[Coding(system=Uri("https://terms.sil-th.org/CodeSystem/cs-eclaim-medication-category"), code=Code(matched_row_16dru.use_status), display=String("ใช้ในโรงพยาบาล"))])]
